...
class Hyperparameters:
    ...
    # Test-Time Training (TTT) with LoRA adapters
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # GQA: expand K/V heads to match Q heads for PyTorch < 2.5 compatibility
        # (enable_gqa kwarg was added in torch 2.5; repeat_interleave works on all versions)
    ...
    def forward_ttt(self, x: Tensor, q_delta: Tensor, v_delta: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        q = self.c_q(x) + q_delta
        k = self.c_k(x)
        v = self.c_v(x) + v_delta
...
class Block(nn.Module):
    ...
    def forward_ttt(self, x: Tensor, x0: Tensor, q_lora, v_lora) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        n = self.attn_norm(x)
        attn_out = self.attn.forward_ttt(n, q_lora(n), v_lora(n))
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x))
        return x
...
class GPT(nn.Module):
    ...
    def forward_ttt(self, input_ids: Tensor, target_ids: Tensor, lora) -> Tensor:
        """Forward pass with LoRA TTT adapters. Returns per-token NLL losses [bsz, seqlen]."""
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        ...
        skips: list[Tensor] = []
        for i in range(self.num_encoder_layers):
            x = self.blocks[i].forward_ttt(x, x0, lora.q_loras[i], lora.v_loras[i])
            skips.append(x)
        for i in range(self.num_decoder_layers):
            bi = self.num_encoder_layers + i
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[bi].forward_ttt(x, x0, lora.q_loras[bi], lora.v_loras[bi])
        x = self.final_norm(x)
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)
        else:
...
class BatchedLinearLoRA(nn.Module):
    """Per-batch-element LoRA for a linear layer. Delta = x @ Aᵀ @ Bᵀ = x @ (BA)ᵀ."""
...
class BatchedTTTLoRA(nn.Module):
    """All LoRA adapters for one batch: lm_head and Q/V per block."""
...
def _find_docs(all_tokens: Tensor, include_next_bos: bool = True) -> list[tuple[int, int]]:
    """Return (start_offset, length) per document, identified by BOS boundaries."""
...
def eval_val_ttt_lora(
    ...
    # Sort by doc length for batching efficiency
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        for ci in range(max_nc):
            # Compute generic window size for this chunk index
            gen_stats = _compute_chunk_window(ci, (ci + 1) * chunk_size, ci + 1, chunk_size, eval_seq_len)
            context_size = gen_stats[1]
            ...
            if needs_train:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ptl = base_model.forward_ttt(x, y, cur_lora)
            else:
            ...
            if needs_train:
                _, _, chunk_offset, _ = _compute_chunk_window(ci, (ci + 1) * chunk_size, ci + 1, chunk_size, eval_seq_len)
                mask = torch.tensor(
        ...
        if rank == 0 and (bi // batch_size) % 20 == 0:
            ...
            if token_count.item() > 0:
                running_bpb = float(loss_sum.item() / math.log(2.0) / byte_sum.item())
...
def main() -> None:
    ...
    if args.ttt_enabled:
        ...
        log0(f"ttt_eval_mode:lora rank:{args.ttt_lora_rank} lr:{args.ttt_lora_lr} "
             f"chunk_size:{args.ttt_chunk_size} batch_size:{args.ttt_batch_size}")
